...
import { computeMatchScore, getBestMatch } from "../modules/bestMatch.js";
import { fetchNearbyPlaces } from "./utils/placesApi.js";
import {performance} from "perf_hooks"
/* testing if getBestMatch algorithm returns the currect place
...
describe("Test(1) - Image of [Cheatmeals Rayners lane] restaurant front", ()=> {
  it("Should match string [Cheatmeals Rayners lane]", async() => {
    const { fullTextAnnotation } = await classifyImageFromBucket(
        ...
        const bestMatch = getBestMatch(places, fullText);
        expect(bestMatch.bestPlaceMatch).toEqual(
...
//     // finding best match for place name and text from image
//     const bestMatch = getBestMatch(places, fullText);
//     expect(bestMatch.bestPlaceMatch).toEqual("Bamboo House");
//   });
// });
...
//     const matchScore = computeMatchScore(textFromImg, target, 2);
...
//     const matchScore1 = computeMatchScore(textFromImg, targets[0], 2);
...
//     const matchScore2 = computeMatchScore(textFromImg, targets[1], 2);
...
//     const matchScore3 = computeMatchScore(textFromImg, targets[2], 2);
